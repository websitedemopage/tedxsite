...
function start() {
    //$('#introvideo').modal('open')
    //document.getElementById("s1").play();
    k2 = setInterval(function () {
        caltime(d1, new Date())
        if(d=="00" && h=="00" && m=="00" && s=="00"){
            ...
            vo=0.1;
            lo=0.9;
            k3 = setInterval(function(){
                if(vo>1.1)
                    clearInterval(k3)
                document.getElementById("videosec").style.opacity=vo;
                document.getElementById("landingsec").style.opacity=lo;
                vo=vo+0.1
                lo=lo-0.1
            },50)
        }
